...
public class SA_CY extends JFrame implements ActionListener
{
    public static void main (String args[])
    {
        SA_CY content = new SA_CY ();
        
        JFrame window = new JFrame ("Surface Area - Cylinder");
    ...
    public SA_CY ()
    {
        ...
        JLabel Instruction = new JLabel ("Surface Area - Cylinder");
        ...
        diagram.add(label);
        JLabel image = new JLabel (createImageIcon ("SA/SA_Cylinder.png"));
        diagram.add(image);
    ...
    public void actionPerformed (ActionEvent e)
    {
    	...
    	{
    		double h = Double.parseDouble(a2.getText());
    		double r = Double.parseDouble(b2.getText());
    		if (h>0 && r>0)
    		{
    			double SA = (2 * Math.PI * r * h) + (2 * Math.PI * r * r)  ;
    			double answer = Math.round(SA);
    			
    			calc.setText("<html>Here are the calculations: <br> <br> SA = (2*PI*r*h) + (2*PI*r*r) <br> SA = (2*PI*"+r+"*"+h+") + (2*PI*"+r+"*"+r+ ") <br> SA = " + SA +" </html>");
    ...
    {
    	java.net.URL imgURL = SA_CY.class.getResource(path);
    	if (imgURL != null)
    	{
