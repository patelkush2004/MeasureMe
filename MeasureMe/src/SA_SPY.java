...
public class SA_SPY extends JFrame implements ActionListener
{
    public static void main (String args[])
    {
        SA_SPY content = new SA_SPY ();
        
        JFrame window = new JFrame ("Surface Area - Square Pyramid");
    ...
    public SA_SPY ()
    {
        ...
        JLabel Instruction = new JLabel ("Surface Area - Square Pyramid");
        ...
        diagram.add(label);
        JLabel image = new JLabel (createImageIcon ("SA/SA_SquarePyramid.png"));
        diagram.add(image);
    ...
    public void actionPerformed (ActionEvent e)
    {
    	if (e.getActionCommand().equals("calculate"))
    	{
    		...
    		if (a>0 && h>0)
    		{
    			double SA = (a*a) + ((2*a)*Math.sqrt(((a*a)/(4))+h*h));
    			double answer = Math.round(SA);
    			
    			calc.setText("<html>Here are the calculations: <br> <br> SA = (a*a) + (2*a)*((a*a/4)+(h*h))^1/2 <br> SA = ("+a+"*"+a+") + (2*"+a+")*(("+a+"*"+a+"/4)+("+h+"*"+h+"))^1/2 <br> SA = " + SA +" </html>");
    ...
    {
    	java.net.URL imgURL = SA_SPY.class.getResource(path);
    	if (imgURL != null)
    	{
